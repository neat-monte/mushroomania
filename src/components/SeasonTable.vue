<template>
  ...
  >
    <thead id="table-header">
      <tr>
        <th class="text-left family" v-on:click="onSort(0, 'family')"
            :priority="sortPriority.indexOf('family')"
            :direction="sortDirectionsAreDescreasing['family'] ? 'down' : 'up'">
          Family
        </th>
        <th class="text-left name" v-on:click="onSort(0, 'name')"
            :priority="sortPriority.indexOf('name')"
            :direction="sortDirectionsAreDescreasing['name'] ? 'down' : 'up'">
          Name
        </th>
        <th class="text-center" v-on:click="onSort(0, 'seasonValue')" 
            :priority="sortPriority.indexOf('seasonValue')"
            :direction="sortDirectionsAreDescreasing['seasonValue'] ? 'down' : 'up'">
          {{header[0].name}}
        </th>
        <th class="text-center" v-on:click="onSort(1, 'seasonValue')">
          {{header[1].name}}
        </th>
        <th class="text-center" v-on:click="onSort(2, 'seasonValue')">
          {{header[2].name}}
        </th>
        <th class="text-center" v-on:click="onSort(3, 'seasonValue')">
          {{header[3].name}}
        </th>
      </tr>
    </thead>
...
export default {
  ...
  setup(props) {
    ...
    const sortPriority = ["family", "name", "seasonValue"]

    // The sort directions of each column, decreasing if true, increasing if false
    const sortDirectionsAreDescreasing = {
      "family": true,
      "name": true,
      "seasonValue": true,
    }

    const sortedData = reactive(props.data);
    sortedData.map(function(e){
      e.sortArray = getSortArray(e);
      e.seasonValue = getValueForSort(e);
      return e;
    })

    const header = reactive(default_header);

    // Call sort once to update the current sort to show mushrooms in spring
    onSort(0, "seasonValue");
    ...
    function onSort(index, target) {
      ...
      sortedData.forEach(function(e){
        cycleArray(e.sortArray, index);
      });

      //Update the season value of each element
      if (index != 0)
      sortedData.map(function(e){
        e.seasonValue = getValueForSort(e);
      });
